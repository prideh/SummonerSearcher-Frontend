  ...
  return (
    <div className={`p-4 w-64 bg-gray-900 rounded-lg shadow-lg z-10 transition-opacity duration-300 ease-in-out ${className}`}>
      ...
      <ul className="text-sm text-gray-400 space-y-1">
        ...
        <li className={requirements.hasLower ? 'text-green-500' : 'text-red-500'}>A lowercase letter</li>
        <li className={requirements.hasNumber ? 'text-green-500' : 'text-red-500'}>A number</li>
        <li className={requirements.hasSpecial ? 'text-green-500' : 'text-red-500'}>A special character (!@#$...)</li>
      </ul>
    </div>
  );
};
