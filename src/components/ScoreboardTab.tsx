    ...
    <div className="bg-black/10 rounded-md">
      ...
      <div className="text-xs">
        {team.map((p, index) => (
          <div 
            key={p.participantId}
            className={`grid grid-cols-[1fr_auto] md:grid-cols-[1fr_90px_60px_40px_200px] gap-x-2 items-center p-1.5 cursor-pointer hover:bg-gray-500/20 transition-colors duration-150 ${p.puuid === searchedPlayerPuuid ? 'bg-blue-500/20' : (index % 2 === 0 ? 'bg-gray-500/5' : 'bg-transparent')}`}
            onClick={() => {
          ...
          >
            ...
            <div className="flex items-center space-x-2 min-w-0">
              <div className="shrink-0">
                <img src={`${CDN_URL}/img/champion/${getCorrectChampionName(p.championName)}.png`} alt={p.championName} className="w-7 h-7 md:w-8 md:h-8 rounded" />
              </div>
