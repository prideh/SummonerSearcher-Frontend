...
const ChangePasswordForm = () => {
  ...
  return (
    <div className="bg-gray-800 p-6 rounded-lg shadow-md w-full max-w-sm">
      ...
      <button onClick={openModal} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:shadow-outline">
        Change Password
      </button>

      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-2xl flex flex-col md:flex-row items-start md:space-x-8">
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  <input type={showOldPassword ? 'text' : 'password'} id="current-password" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} className="shadow appearance-none border border-gray-600 rounded w-full py-2 px-3 bg-gray-700 text-gray-300 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500 pr-16" required autoComplete="current-password" />
                  <button type="button" onClick={() => setShowOldPassword(!showOldPassword)} tabIndex={-1} className="absolute inset-y-0 right-0 pr-3 flex items-center text-sm font-bold text-gray-400 hover:text-blue-400 focus:outline-none">{showOldPassword ? 'Hide' : 'Show'}</button>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  <input type={showPassword ? 'text' : 'password'} id="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="shadow appearance-none border border-gray-600 rounded w-full py-2 px-3 bg-gray-700 text-gray-300 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500 pr-16" required autoComplete="new-password" />
              ...
              <div className="mb-6">
                ...
                <input type={showPassword ? 'text' : 'password'} id="confirm-new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="shadow appearance-none border border-gray-600 rounded w-full py-2 px-3 bg-gray-700 text-gray-300 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500" required autoComplete="new-password" />
              </div>
              ...
              <div className="flex items-center justify-end space-x-4">
                ...
                <button type="submit" disabled={isSubmitting} className="bg-blue-500 hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded focus:shadow-outline flex items-center">
